...
from simsiam.criterion import SupSimSiamLoss
...
parser.add_argument(
    "--arch", default="resnet18", help="model name is used for training"
)

parser.add_argument("--feat_dim", default=2048, type=int, help="feature dimension")
parser.add_argument(
    "--num_proj_layers", type=int, default=2, help="number of projection layer"
)
...
def main():
    ...
    train_transforms = transforms.Compose(
        [
            transforms.RandomResizedCrop(args.img_dim, scale=(0.2, 1.0)),
    ...
    criterion = SupSimSiamLoss(f'cuda:{args.gpu}')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    progress = ProgressMeter(
        len(train_loader), [batch_time, losses], prefix="Epoch: [{}]".format(epoch)
    )
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        loss = criterion(
            p=torch.cat([outs["p1"], outs["p2"]], dim=0),
            z=torch.cat([outs["z1"], outs["z2"]], dim=0),
